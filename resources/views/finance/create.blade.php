    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ url('financea/create') }}">
                        @csrf

                        <div class="row mb-3">
                            <label for="name" class="col-md-4 col-form-label text-md-end">{{ __('Name User') }}</label>

                            <div class="col-md-6">
                                <select class="form-control" name="id_user" id="id_user">
                                        <option>Pilih Nama Akun</option>
                                    @foreach ($saldo as $sd)
                                        <option value="{{ $sd->id }}">ID{{ $sd->id }} - {{$sd->name}}</option>
                                    @endforeach
                                </select>
                                @error('name')
